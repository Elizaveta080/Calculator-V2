    ...
    public static String calc(String input) throws ScanExc {
        ...
        /*if (isInt(exp[0])) {
            if (operationIndex == 1 && exp.length == 3) {
                result = (calcArab(Integer.parseInt(exp[0]),
                        Integer.parseInt(exp[2]),
                        String.valueOf(operationIndex))) + "";
            }
            if (operationIndex == 2 && exp.length == 5) {
                result = (calcArab(Integer.parseInt(exp[0] + exp[1]),
                        Integer.parseInt(exp[3] + exp[4]),
                        String.valueOf(operationIndex))) + "";
            }
        ...
        if (exp.length == 3) {
            ...
            //TODO пройдись ниже по if дебаггером, чтобы понимать почему не работало
            if (operationIndex == 1 && exp.length == 3) {
            ...
            int preResult = calcArab(x, y, exp[operationIndex]); //TODO и тут знак не верный индекс у массива было, вся проблема была только в неправильных индексах, конвертор скорей всего рабочий тот был
